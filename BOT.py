...
def get_stato_utente(uid):
    if os.path.exists(LOG_FILE):
        try:
            with open(LOG_FILE, "r") as f: return json.load(f).get(str(uid), {"CAPITALE":0.0, "SIMBOLO":"", "ATTIVO":False, "TRADE_APERTO":False})
        except: pass
...
def start(m):
    ...
    salva_stato_utente(m.chat.id, {"CAPITALE": 0.0, "SIMBOLO": "", "ATTIVO": False, "TRADE_APERTO": False})
...
def handle(m):
    ...
    if s["CAPITALE"] == 0:
        ...
        bot.reply_to(m, "🚀 Motore di scansione attivato.")
